...
(function() {
  ...
  function renderVideoList(videos) {
    ...
    if (videos.length === 0) {
      videoListContainer.innerHTML = '<p class="no-videos">当前页面没有找到视频</p>';
      return;
    }
    ...
    videos.forEach(video => {
      ...
      title.textContent = video.title || video.src || '未知视频';
      
      // Video source URL
      const src = document.createElement('p');
      src.className = 'video-src';
      src.textContent = video.src ? truncateUrl(video.src) : '无源地址';
 ...
 async function openMiniplayerForVideo(video) {
  ...
  async function init() {
    // Show loading state
    const videoListContainer = document.getElementById('video-list');
    videoListContainer.innerHTML = '<p>正在搜索页面中的视频...</p>';
